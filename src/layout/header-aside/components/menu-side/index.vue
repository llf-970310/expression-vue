<template>
  <div class="d2-layout-header-aside-menu-side">
    <el-menu
      ...
      <template v-for="(menu, menuIndex) in curAsideList">
        <d2-layout-header-aside-menu-item v-if="menu.children === undefined" :menu="menu" :key="menuIndex"/>
        <d2-layout-header-aside-menu-sub v-else :menu="menu" :key="menuIndex"/>
      </template>
    </el-menu>
    <div v-if="curAsideList.length === 0 && !asideCollapse" class="d2-layout-header-aside-menu-empty"
  ...
  export default {
    ...
    watch: {
      ...
      role: function (newVal, oldVal) {
        console.log('role has changed to ' + newVal)
        this.curAsideInit()
      }
    },
    mounted() {
